pro Single_Channel_TM
  ...
  xmanager, 'Single_Channel_TM', tlb, /no_block
end
pro Single_Channel_TM_event, ev
  ;获取textIn控件的id
  ...
  case uname of
    ...
    'buttonRun' : begin  ;如果点击Run按钮，则执行以下操作
      ...
      K2 = 1260.56
      T = K2/alog(K1/T6+1)
      r = 1/(T6*C2/T^2*((11.4350^4*T6/C1)+(1/11.4350)))
      d = -r*T6 + T
      a11 = 0.14714
      a12 = -0.15583
      a13 = 1.1234
      ...
      Ts = r*(EM^(-1)*(Fi1*T6+Fi2)+Fi3)+d
      TC = Ts-273.15
      write_image, fileOut, 'TIFF', PV ;保存图像
